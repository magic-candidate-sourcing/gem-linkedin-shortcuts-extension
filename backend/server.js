...
<html lang="en">
  ...
  <body>
    ...
    <ul>
      <li>LinkedIn profile page context (such as profile URL/handle and visible name) when a user triggers an action.</li>
      <li>User-configured extension settings (shortcuts, backend URL, and optional defaults).</li>
      <li>Action logs generated by extension/backend for troubleshooting.</li>
    </ul>
    <h2>What data the extension does not store</h2>
    <ul>
      <li>Gem API keys and Ashby API keys are not stored in the extension.</li>
      <li>These keys are stored on your organization's backend service.</li>
    </ul>
    <h2>How data is used</h2>
    <p>Data is used only to execute user-initiated recruiting actions.</p>
    <h2>Data sharing</h2>
    <ul>
    ...
    <ul>
      ...
      <li>Optional backend shared-token validation can be enabled.</li>
